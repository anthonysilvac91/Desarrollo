...
export default function ServiceDrawer({ service, onClose, readOnly = false }: ServiceDrawerProps) {
  ...
  // Nullable-safe view of the service, usable before the early return below
  ...
  const videoAttachments = attachments.filter(a => a.media_type === "VIDEO" || a.file_type?.startsWith("video/"));
  const documentAttachments = attachments.filter(a => a.file_type && !a.file_type.startsWith("image/") && !a.file_type.startsWith("video/"));
  ...
  useEffect(() => {
    pinch.reset();
    setCfDownload(null);
    if (selectedIsVideo && selectedMedia?.id && serviceData) {
      const attId = selectedMedia.id;
      setVideoPlayback(null);
      setLoadingVideoId(attId);
      servicesService.getVideoPlaybackUrl(serviceData.id, attId)
  ...
  const handleLightboxSwipeEnd = (x: number) => {
    ...
    if (deltaX < 0 && selectedMediaIndex < mediaAttachments.length - 1) setSelectedMediaIndex(selectedMediaIndex + 1);
    if (deltaX > 0 && selectedMediaIndex > 0) setSelectedMediaIndex(selectedMediaIndex - 1);
    setTouchStartX(null);
  };
  ...
  const handleRequestCfDownload = async () => {
    ...
    try {
      const data = await servicesService.getVideoDownloadUrl(currentService.id, selectedMedia.id);
      setCfDownload(data);
  ...
  const handlePdfPreview = async (att: { id?: string; file_name?: string | null }) => {
    ...
    try {
      const { url } = await servicesService.getAttachmentDownloadUrl(currentService.id, att.id);
  ...
  const handleDocumentDownload = async (att: { id?: string; file_name?: string | null; file_type?: string }) => {
    if (!att.id) return;
    try {
      const { url, file_name } = await servicesService.getAttachmentDownloadUrl(currentService.id, att.id);
      const a = document.createElement("a");
      a.href = url;
  ...
  return (
    <Drawer isOpen={!!service} onClose={onClose} panelClassName="bg-app-bg">
      <div className="flex flex-col min-h-full">
        ...
        <AttachmentUploadSummary service={currentService} />
              ...
              {mediaAttachments.length > 1 && (
                <div className="w-full max-w-sm flex items-center gap-2">
                  <button
                    ...
                    className="shrink-0 p-2 rounded-full bg-surface shadow-lg border border-border-theme/20 active:scale-90 transition-all disabled:opacity-30"
                    aria-label="Anterior"
                  >
                ...
                <div className="flex items-start gap-3 bg-surface/60 backdrop-blur-sm rounded-2xl p-4 border border-border-theme/20">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-subtitle/70 font-medium">
                      {currentService.worker?.name} · {formatDate(currentService.created_at)}
                      {!selectedIsVideo && (
