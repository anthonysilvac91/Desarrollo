...
interface UploadQueueContextValue {
  ...
  attachFileToUpload: (localId: string, file: File) => Promise<boolean>;
}
...
export function UploadQueueProvider({
  ...
  const progressReportRef = useRef(
    new Map<string, { progress: number; at: number }>(),
  );
  ...
  useEffect(() => {
    localStorage.setItem(
      STORAGE_KEY,
      JSON.stringify(items.map(persistedShape)),
    );
  }, [items]);
  ...
  const pollStreamConfirm = useCallback(
    function pollStreamConfirm(
      localId: string,
      serviceId: string,
      uploadId: string,
      attempt = 0,
      ...
      if (attempt > 30) {
        console.error("[upload] Timeout esperando procesamiento de CF Stream", {
          localId,
          uploadId,
          serviceId,
          attempts: attempt,
      ...
      setTimeout(async () => {
        try {
          ...
          console.error("[upload] Error al confirmar video en polling", {
            localId,
            uploadId,
            serviceId,
            attempt,
          ...
          patchItem(localId, {
            status: "failed",
            error: "Error al confirmar el video procesado.",
          });
        }
      }, 3000);
    },
  ...
  const startUpload = useCallback(
    (item: UploadQueueItem) => {
      if (
        ...
        !item.intent ||
        !item.uploadId ||
        uploadsRef.current.has(item.localId)
      )
        return;
      patchItem(item.localId, { status: "uploading", error: undefined });
      uploadService
        .markStarted(item.serviceId, item.uploadId)
        .catch(() => undefined);
      ...
      const upload = createTusUpload(item.file, item.intent, {
        onProgress: (bytesUploaded, bytesTotal) => {
          ...
          if (
            !last ||
            progress - last.progress >= 10 ||
            Date.now() - last.at > 30000
          ) {
            progressReportRef.current.set(item.localId, {
              progress,
              at: Date.now(),
            });
            uploadService
        ...
        onSuccess: async () => {
          ...
          patchItem(item.localId, {
            status: "confirming",
            progress: 100,
            bytesUploaded: item.sizeBytes,
          });
          try {
            await uploadService.updateProgress(
              item.serviceId,
              item.uploadId!,
              100,
              "UPLOADED",
            ...
            if (result?.status === "PROCESSING") {
              patchItem(item.localId, {
                status: "confirming",
                progress: 100,
                error: "El video se está procesando...",
              });
            ...
            await Promise.all([
              ...
              queryClient.invalidateQueries({
                queryKey: ["service", item.serviceId],
              }),
            ]);
          } catch (error: unknown) {
            console.error(
              "[upload] Error al confirmar carga tras TUS success",
              {
                localId: item.localId,
                uploadId: item.uploadId,
                serviceId: item.serviceId,
  ...
  useEffect(() => {
    ...
    const next = items.find(
      (item) => item.status === "queued" && item.file && item.intent,
    );
    if (next) startUpload(next);
  }, [items, concurrency, startUpload]);

  const enqueueVideos = useCallback(
    ({ serviceId, files, intents = [] }: EnqueueInput) => {
      const intentByClientId = new Map(
  ...
  const resumeUpload = useCallback(
    (localId: string) => {
      ...
      if (!item?.intent) {
        patchItem(localId, {
          status: "needs_file",
          error: "Selecciona nuevamente el archivo para renovar la carga.",
        });
        return;
      }
  ...
  const cancelUpload = useCallback(
    async (localId: string) => {
      ...
      if (item?.uploadId) {
        await uploadService
          .cancel(item.serviceId, item.uploadId)
          .catch(() => undefined);
      }
  ...
  const attachFileToUpload = useCallback(
    async (localId: string, file: File) => {
      ...
      if (
        ...
        patchItem(localId, {
          error: "El archivo seleccionado no coincide con la carga original.",
        });
        return false;
      }
      if (!item.uploadId) {
        patchItem(localId, {
          file,
          status: "failed",
          error: "No se pudo renovar la carga.",
        });
        return false;
      }
      try {
