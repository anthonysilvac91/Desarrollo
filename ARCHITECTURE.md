...
| **EXTERNAL** | Usuario de owner | Persona asociada a un `Owner` (empresa cliente). Solo ve `Assets` vinculados a su owner y `Services` marcados como publicos (`is_public: true`). |
...
- **Usuarios externos**: el rol `EXTERNAL` representa a un usuario asociado a un `Owner`. Solo ve assets de su owner y services publicos.
- **Visibilidad externa**: `is_public: true` en un `Service` lo hace visible para el usuario `EXTERNAL` del owner dueno del asset.
...
- **Backend**: `AuthGuard` protege endpoints y extrae `organization_id`, `role` y `owner_id` del token para la logica de negocio.
