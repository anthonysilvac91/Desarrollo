...
describe('AssetsController public route contract', () => {
    ...
    const routes = Object.getOwnPropertyNames(AssetsController.prototype)
      ...
      .map((name) => ({
        path: Reflect.getMetadata(
          PATH_METADATA,
          AssetsController.prototype[name],
        ),
  ...
  it('EXTERNAL recibe ForbiddenException al intentar crear activo', () => {
    ...
    expect(() => controller.create({} as any, req as any, undefined)).toThrow(
      ForbiddenException,
    );
  ...
  it('assigns and removes owners through the canonical service calls', () => {
    const assetsService = {
      assignOwner: jest
        .fn()
        .mockReturnValue({ id: 'asset-1', owner_id: 'owner-1' }),
      removeOwner: jest
        .fn()
        .mockReturnValue({ id: 'asset-1', owner_id: 'owner-1' }),
    } as any;
    ...
    expect(controller.assignOwner('asset-1', 'owner-1', req)).toEqual({
      id: 'asset-1',
      owner_id: 'owner-1',
    });
    expect(controller.removeOwner('asset-1', 'owner-1', req)).toEqual({
      id: 'asset-1',
      owner_id: 'owner-1',
    });
    expect(assetsService.assignOwner).toHaveBeenCalledWith(
      'asset-1',
      'owner-1',
      'org-1',
    );
    expect(assetsService.removeOwner).toHaveBeenCalledWith(
      'asset-1',
      'owner-1',
      'org-1',
    );
  ...
  it('delega filter-options al servicio con tenant y rol', () => {
    ...
    expect(assetsService.getFilterOptions).toHaveBeenCalledWith(
      'org-1',
      'WORKER',
      undefined,
      'worker-1',
