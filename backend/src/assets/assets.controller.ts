...
export class AssetsController {
  ...
  @UseInterceptors(
    FileInterceptor('photo', imageUploadOptions(ASSET_IMAGE_MAX_BYTES)),
  )
  @ApiConsumes('multipart/form-data')
  @ApiOperation({
    summary: 'Crear un nuevo activo',
    description: 'Disponible para Worker y Admin.',
  })
  create(
  ...
  @Get()
  @ApiOperation({
    summary: 'Listar activos segun rol',
    description:
  ...
  assignOwner(
    ...
    if (req.user.role !== 'ADMIN')
      throw new ForbiddenException('Solo ADMIN puede asignar');
  ...
  removeOwner(
    ...
    if (req.user.role !== 'ADMIN')
      throw new ForbiddenException('Solo ADMIN puede desasignar');
  ...
  toggleStatus(
    ...
    if (!['ADMIN', 'SUPER_ADMIN'].includes(req.user.role)) {
      throw new ForbiddenException(
        'No tienes permiso para cambiar el estado de activos',
      );
    }
  ...
  remove(
    @Param('id') id: string,
    @Body() body: { deleteServices?: boolean } | undefined,
    @Request() req,
  ) {
    if (req.user.role !== 'ADMIN' && req.user.role !== 'SUPER_ADMIN')
      throw new ForbiddenException('No tienes permiso para borrar activos');
