...
describe('DashboardService tenant scoping', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: StoredFilesService, useValue: { resolveFileUrl: jest.fn(), resolveFileUrlOrRef: jest.fn(), resolveFileUrlForOrg: jest.fn().mockResolvedValue(null) } },
      ],
    }).compile();
  ...
  it('EXTERNAL con owner_id queda filtrado por su owner', async () => {
    ...
    expect(prisma.asset.count).toHaveBeenCalledWith({
      where: { organization_id: 'org-1', owner_id: 'owner-1', is_active: true },
    });
  ...
  it('la evolucion usa el rango de fecha seleccionado', async () => {
    ...
    await service.getStats(
      ...
      undefined,
      { startDate: '2026-06-01T00:00:00.000Z', endDate: '2026-06-10T23:59:59.999Z' },
    );
  ...
  it('ADMIN: total_workers y total_admins cuentan solo usuarios activos', async () => {
    ...
    expect(prisma.user.count).toHaveBeenCalledWith({
      where: { organization_id: 'org-1', role: Role.WORKER, is_active: true },
    });
    expect(prisma.user.count).toHaveBeenCalledWith({
      where: { organization_id: 'org-1', role: Role.ADMIN, is_active: true },
    });
  });
});
